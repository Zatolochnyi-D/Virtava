...
class TrackerServer:
    ...
    def __heartbeat_indefinitely(self, reply_socket: zmq.SyncSocket, connection_timeout: int):
        ...
        reply_socket.close()  # TODO: Handle possible errors. Check ZMQ manual for any.
    ...
    def send(self, result: Message): # TODO: Look up if this thing is good enough. Like, can I just directly pass things to send to socket, ot should I wrap it with thread or something?
        try:
            self.__broadcast_socket.send(result.SerializeToString()) # Look up in docs all possible problems for socket.send method.
        except zmq.ZMQError:
            self.__logger.error("Error occured on sending message.") # TODO: look up how to properly detect socket closed on sending.

    def stop(self):
        self.__logger.debug("Started closing TrackerServer.")
        self.__broadcast_socket.close() # TODO: Handle possible errors. Check ZMQ manual for any.
